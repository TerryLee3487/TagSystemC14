...
void Controller::cut() {
    ...
    // (note: this could be done just by moving pointers,
    // but we want to test the implementation of the move constructor
    // and move assignment operator, so, we will use them instead)
    if (clip == nullptr) {
